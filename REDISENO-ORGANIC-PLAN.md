...
> **Estado (2026-08-26):** Fase 0 y Fase 1 (1a-1e) **completadas y fusionadas a `main`**.
> Fases 2-6 **aprobadas, sin empezar**. Este documento es el plan de implementación completo,
...
| **2** | Migraciones 1-3 (vitals, próxima dosis, lotes/vencimiento) + UI en los diálogos existentes de Historial e Inventario. | ⬜ Pendiente — siguiente fase a ejecutar. |
| **3** | Migración 4 (lista de espera) + UI dentro de Agenda (1h, wiring en 1i). | ⬜ Pendiente. |
...
Con Fases 0 y 1 completas y verificadas, la **Fase 2** arranca con:
`npx supabase migration new historial_signos_vitales` (y las otras dos migraciones de la
tabla de arriba), extender `types/dominio.ts`, y agregar los campos a los diálogos de
Historial e Inventario ya existentes. Se commitea y se verifica sola antes de seguir con la
Fase 3 — mismo ritmo por fases con revisión entre cada una, por pedido explícito del cliente.
